...
export const signUpService = async (req, res) => {
  ...
  if (!firstName || !lastName || !email || !password || !gender || !DOB || !mobileNumber) {
...
export const gmailSignUpService = async (req, res) => {
  ...
  const randomPassword = hashSync(Math.random().toString(36).slice(-8), +process.env.SALT);
...
export const refreshToken = async (req, res) => {
  ...
  if (user.changeCredentialTime && data.iat * 1000 < user.changeCredentialTime.getTime()) {
    return res.status(403).json({ message: "Token expired due to credential change, please login again" });
  }
